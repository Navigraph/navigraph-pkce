...
describe("@navigraph/pkce", () => {
    ...
    test("code_verifier doesn't have [=+/]", () => {
      const challengePair = pkce();
      expect(challengePair.code_verifier).toEqual(expect.not.stringContaining("="))
      expect(challengePair.code_verifier).toEqual(expect.not.stringContaining("+"))
      expect(challengePair.code_verifier).toEqual(expect.not.stringContaining("/"))
    });
    ...
    test("code_challenge to be base64 uri-encoded sha256(code_verifier)", () => {
      ...
      const challengePair = pkce();
      expect(challengePair.code_challenge).toEqual(generateChallenge(challengePair.code_verifier))
    });
